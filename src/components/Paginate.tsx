...
type PaginateAction = (event: React.MouseEvent<HTMLButtonElement>, type: string) => void
...
const Paginate: FC = () => {
  ...
  return (
    <div className="mt-6 flex justify-center items-center">
      {state.page > 1 && (
        <button className="w-6 md:w-8 h-6 md:h-8 paginate-btn" type="button" onClick={(e) => handleClickPagenate(e, 'prev')}>
      ...
      {state.page < state.totalPage && (
        <button className="w-6 md:w-8 h-6 md:h-8 paginate-btn" type="button" onClick={(e) => handleClickPagenate(e, 'next')}>
